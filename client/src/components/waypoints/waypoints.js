...
;(function() {
  ...
  Context.prototype.handleScroll = function() {
    ...
    for (var axisKey in axes) {
      ...
      Object.keys(this.waypoints[axisKey])
        .map(key => this.waypoints[axisKey][key])
        .forEach(waypoint => {
          if (waypoint.triggerPoint < window.scrollY) {
            lastWaypoint = waypoint;
          } else if (!nextWaypoint && waypoint.triggerPoint > window.scrollY) {
            nextWaypoint = waypoint;
          }
        })

      if (nextWaypoint && lastWaypoint && nextWaypoint.scrollHandler){
        var event = {
          newScroll: axis.newScroll,
          oldScroll: axis.oldScroll,
          direction,
          lastWaypoint,
          nextWaypoint
        }
