...
export default {
  name: "d3Impl",
  mixins: [Visualization({staticDataset: "playfair_nums_def"})],
  mounted() {
  },
  props: {
    slideNumber: {
  ...
  methods: {
    ...
    generateSvg() {
      ...
      const maxOn = prop => Math.max(...csvData.map(d => d[prop]));
      const minOn = prop => Math.min(...csvData.map(d => d[prop]));
...
.chart {
  ...
  box-shadow: inset 0 0 0 4px #FCE2B0,
